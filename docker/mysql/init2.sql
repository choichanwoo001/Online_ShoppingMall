-- USERS 테이블 (user_id PK 유지, login_id 추가)
CREATE TABLE `USERS` (
...
-- USER_DETAIL 테이블 (user_id 제거, login_id로 대체)
...
-- USER_COUPON 테이블 (user_id 제거, login_id로 대체)
...
-- Inquiry 테이블 (user_id 제거, login_id로 대체)
...
-- Cart 테이블 (user_id 제거, login_id로 대체)
CREATE TABLE `Cart` (
...
-- default_user_address 테이블 (user_id 제거, login_id로 대체)
...
-- ORDERS 테이블 (user_id 제거, login_id로 대체)
CREATE TABLE `ORDERS` (
...
-- review_comment 테이블 (user_id 제거, login_id로 대체)
...
-- review 테이블 (user_id 제거, login_id로 대체)
CREATE TABLE `review` (
...
-- PAYMENT 테이블 (user_id 제거, login_id로 대체)
...
-- user_terms_agreement 테이블 (user_id 제거, login_id로 대체)
...
-- report 테이블 (user_id 제거, login_id로 대체)
CREATE TABLE `report` (
...
-- ADMIN_DETAIL 테이블 (user_id 제거, login_id로 대체)
...
-- review_like 테이블 (user_id 제거, login_id로 대체)
...
-- mileage_history 테이블 (user_id 제거, login_id로 대체)
...
-- WISH_LIST 테이블 (user_id 제거, login_id로 대체)
...
-- LOGIN_HISTORY 테이블 (user_id 제거, login_id로 대체)
...
-- mileage 테이블 (user_id 제거, login_id로 대체)
...
-- refund_account 테이블 (user_id 제거, login_id로 대체)
...
CREATE TABLE `Order_state_code` (
                                    ...
                                    PRIMARY KEY (`id`)
) ENGINE=InnoDB COMMENT='주문 상태 코드';
